...
def format_row_data(field_labels, field_data):
    ...
    return list(zip(field_labels, field_data))
...
class DataEditor(QtGui.QWidget):
    def __init__(self):
        ...
        # left panel
        load_excel_button = QtGui.QPushButton("导入 表格")
        load_gdrive_button = QtGui.QPushButton("导入 Google文档")
        load_gdrive_button.setEnabled(False)
        load_json_button = QtGui.QPushButton("导入 json")
        ...
        export_excel_button = QtGui.QPushButton("导出 表格")
        # export_excel_button.setEnabled(False)
        export_gdrive_button = QtGui.QPushButton("导出 Google文档")
        export_gdrive_button.setEnabled(False)
        export_json_button = QtGui.QPushButton("导出 json")
        ...
        self.line_editor_area_list.addItem("点击上方按钮载入数据")
        self.line_editor_area_list.currentRowChanged.connect(self.line_selected)

        insert_new_line_button = QtGui.QPushButton("插入")
        delete_line_button = QtGui.QPushButton("删除")
        copy_line_button = QtGui.QPushButton("复制")
        ...
        # right panel
        field_name_label = QtGui.QLabel("字段名")
        field_name_label.setAlignment(QtCore.Qt.AlignTop)
        field_type_label = QtGui.QLabel("类型")
        field_type_label.setAlignment(QtCore.Qt.AlignTop)
        field_value_label = QtGui.QLabel("值")
        ...
        reset_item_button = QtGui.QPushButton("初始化")
        save_item_button = QtGui.QPushButton("保存")
        close_item_button = QtGui.QPushButton("关闭")
    ...
    def display_item_view(self):
        ...
        self.item_list = []
        for (field, field_data) in item_dict.items():
    ...
    def load_from_excel(self):
        file_name = QtGui.QFileDialog.getOpenFileName(self, "excel文件打开", "", "excel files (*.xls *.xlsx)")
        if file_name != "":
            self.dict_list = []
            self.sheet_names = []
            self.schema_dict = []
    ...
    def load_from_json(self):
        file_name = QtGui.QFileDialog.getOpenFileName(self, "json文件打开", "", "json files (*.json)")
        if file_name != "":
            with open(str(file_name)) as json_file:
                self.dict_list = []
                self.sheet_names = []
                self.schema_dict = []
                self.clear_tab_bar()
    ...
    def save_to_json(self):
        file_name = QtGui.QFileDialog.getSaveFileName(self, "json文件保存", "", "json files (*.json)")
        json_sheets = OrderedDict(list(zip(self.sheet_names, self.dict_list)))
    ...
    def save_to_excel(self):
        file_name = QtGui.QFileDialog.getSaveFileName(self, "excel文件保存", "", "xls files (*.xls)")
        xls = xlwt.Workbook(encoding='UTF-8')
        for order in range(len(self.sheet_names)):
            sheet = xls.add_sheet(self.sheet_names[order])
            schema = list(self.schema_dict[order].keys())
            for col in range(len(self.schema_dict[order])):
                sheet.write(0, col, schema[col])
            for row in range(len(self.dict_list[order])):
                row_dict = list(self.dict_list[order][row].values())
                for col in range(len(self.schema_dict[order])):
                    sheet.write(row + 1, col, json.dumps(row_dict[col], ensure_ascii=False).strip('"').strip("'"))
        xls.save(file_name)
